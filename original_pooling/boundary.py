...
def adjacency_from_boundaries(boundaries: Boundaries, dim):
    if dim >= 1:
        D0 = np.diag(np.sum(np.abs(boundaries.B1), axis=1))
        A0 = np.abs(D0 - np.matmul(boundaries.B1, boundaries.B1.T))
    else:
        A0 = None
    if dim >= 2:
        D1 = np.diag(np.sum(np.abs(boundaries.B2), axis=1))
        A1 = np.abs(D1 - np.matmul(boundaries.B2, boundaries.B2.T))
    else:
        A1 = None
    if dim >= 3:
        D2 = np.diag(np.sum(np.abs(boundaries.B3), axis=1))
        A2 = np.abs(D2 - np.matmul(boundaries.B3, boundaries.B3.T))
    else:
        A2 = None
...
def boundary_from_simplices(simplices: Simplices, dim) -> Boundaries:
    B0 = None
    if dim >= 1:
        B1 = np.zeros([len(simplices.nodes), len(simplices.edges)])
    ...
    # Build B2
    if dim >= 2:
        B2 = np.zeros([len(simplices.edges), len(simplices.cycles)])
    ...
    # Build B3
    if dim >= 3:
        B3 = np.zeros([len(simplices.cycles), len(simplices.tetra)])
