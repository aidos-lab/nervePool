...
def _buildSimplexListADJ(scomplex):
    """
...
def _buildSimplexListBDS(scomplex):
    """
...
def _buildBoundaries(scomplex):
    if scomplex.boundaries is None:
        # print("Using list of simplices to construct boundaries...")
        scomplex.B0 = None
        # Build B1
        if scomplex.dim >= 1:
            scomplex.B1 = np.zeros([len(scomplex.simplices[0]), len(scomplex.simplices[1])])
        ...
        # Build B2
        if scomplex.dim >= 2:
            scomplex.B2 = np.zeros([len(scomplex.simplices[1]), len(scomplex.simplices[2])])
        ...
        # Build B3
        if scomplex.dim >= 3:
            scomplex.B3 = np.zeros([len(scomplex.simplices[2]), len(scomplex.simplices[3])])
